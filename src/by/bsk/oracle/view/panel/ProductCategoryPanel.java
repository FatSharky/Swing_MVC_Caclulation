...
public class ProductCategoryPanel extends JPanel {
	...
	private static final Logger logger = LogManager.getLogger(ProductCategoryPanel.class);
	...
	private static ProductCategoryPanel productPanel;
	...
	private void createPanel(User user) {
		...
		label = new JLabel(resourceBundle.getString(Field.PRODUCT_PANEL_LABEL) + "  ");
		label.setFont(new Font("Tahoma", Font.BOLD, 16));
		menuBar.add(label);

		bAddProduct = createButton(menuBar, resourceBundle.getString(Field.PRODUCT_PANEL_ADD_INFO), GifPlace.GIF_ADD);
		...
		bUpdateProduct = createButton(menuBar, resourceBundle.getString(Field.PRODUCT_PANEL_EDIT_INFO),
				GifPlace.GIF_UPDATE);
		...
		bDeleteButton = createButton(menuBar, resourceBundle.getString(Field.PRODUCT_PANEL_DELETE_INFO),
				GifPlace.GIF_DELETE);
	...
	public static ProductCategoryPanel getInstance() {
		if (productPanel == null) {
			synchronized (ProductCategoryPanel.class) {
				if (productPanel == null) {
					productPanel = new ProductCategoryPanel();
				}
			}
		}
		return productPanel;
